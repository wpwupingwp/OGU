...
class PrimerWithInfo(SeqRecord):
    def __init__(self, seq='', quality='', start=0, coverage=0, loc=None,
                 avg_mismatch=0, reverse_complement=False):
        ...
        self.loc = self.annotations['loc'] = loc
        self.avg_mid_loc = 0
    ...
    def update_id(self):
        ...
        if self.loc is not None and len(self.loc) != 0:
            mid = [(i[0]+i[1])/2 for i in self.loc]
            self.avg_mid_loc = int(np.average(mid))
        self.id = ('AvgMidLocation({:.0f})-Tm({:.2f})-Coverage({:.2%})-'
                   'Start({})-End({})'.format(self.avg_mid_loc, self.tm,
                                              self.coverage, self.start,
                                              self.end))
...
class Pair:
    ...
    def __init__(self, left, right, alignment):
        ...
        new_l = list()
        new_r = list()
        for l, r in zip(self.left.loc, self.right.loc):
            # [start, end]
            length = r[0] - l[1]
        ...
        self.left.loc = new_l
        self.right.loc = new_r
        # recalculate coverage due to dropping some records
        self.left.coverage = len(self.left.loc) / rows
        self.right.coverage = len(self.right.loc) / rows
        self.left.update_id()
...
def prepare(fasta):
    ...
    Generate list of fasta record  without gap
    """
    data = list()
    record = ['id', 'sequence']
    ...
    seq_str = [i[1] for i in data]
    no_gap = [i.replace('-', '') for i in seq_str]
...
# profile
def validate(primer_candidate, sequences, arg):
    """
    Use regex.fuzzymatch instead of BLAST, return List[PrimerWithInfo]
    """
    rows = len(sequences)
    max_mismatch = arg.mismatch
    primer_verified = list()
    for query in primer_candidate:
        pattern = re.compile(r'({}){{e<={}}}'.format(str(query.seq),
                                                     max_mismatch))
        locations = list()
        mismatches = list()
        for text in sequences:
            ...
            for match in re.finditer(pattern, text):
                ...
                loc = match.span()
                match_info.append([mismatch, loc])
                if mismatch == 0:
                    perfect_loc = loc
                    perfect_match += 1
            if perfect_match > 1:
                ...
                mismatches.append(0)
                locations.append(perfect_loc)
            elif len(match_info) == 1:
                mismatches.append(match_info[0][0])
                locations.append(match_info[0][1])
        hits = len(locations)
        coverage = hits / rows
        if coverage >= arg.coverage:
            query.coverage = coverage
            query.loc = locations
            query.avg_mismatch = np.average(mismatches)
            query.update_id()
            primer_verified.append(query)
...
def main():
    ...
    name, alignment, sequences = prepare(arg.input)
    rows, columns = alignment.shape
...
""".format(max(seq_count), arg.resolution))
    ...
    # validate
    primer_verified = validate(primer_candidate, sequences, arg)
    # pick pair
